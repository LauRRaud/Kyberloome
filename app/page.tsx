...
export default function Home() {
  return <>
    ...
    <main id="sisu">
      ...
      <section id="teenused" className="section wrap"><div className="section-heading"><div><p className="eyebrow">01 / MIDA ME LOOME</p><h2>Hea idee väärib<br/><span className="muted">toimivat lahendust.</span></h2></div><p>Aitame ehitada digitaalseid tööriistu,<br/>mis teevad päriselt töö ära.</p></div><div className="services">{services.map(([title, description], i) => <article className="service" key={title}><span className="service-index" aria-hidden="true">0{i + 1}</span><div className="service-copy"><h3>{title}</h3><p>{description}</p></div></article>)}</div></section>
      ...
      <section id="protsess" className="section wrap"><div className="section-heading"><div><p className="eyebrow">03 / KUIDAS TÖÖTAME</p><h2>Ideest valmis<br/><span className="muted">lahenduseni.</span></h2></div><p>Selge protsess. Vahetu suhtlus.<br/>Igas etapis tead, kuhu liigume.</p></div><div className="steps">{steps.map(([title, text], i) => <article key={title}><div className="step-number">0{i + 1}<span aria-hidden="true">{i === 3 ? '↗' : '→'}</span></div><h3>{title}</h3><p>{text}</p></article>)}</div></section>
