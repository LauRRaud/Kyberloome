...
export default function Home() {
  return <>
    ...
    <main id="sisu">
      ...
      <section id="teenused" className="section wrap"><div className="section-heading"><div><p className="eyebrow">01 / MIDA ME LOOME</p><h2>Hea idee väärib<br/>toimivat lahendust.</h2></div><p>Aitame ehitada digitaalseid tööriistu,<br/>mis teevad päriselt töö ära.</p></div><div className="services">{services.map(([title, description], i) => <article className="service" key={title}><span className="service-index" aria-hidden="true">0{i + 1}</span><div className="service-copy"><h3>{title}</h3><p>{description}</p></div></article>)}</div></section>
