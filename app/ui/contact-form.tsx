...
export default function ContactForm() {
 ...
 <div className="form-row"><label>Nimi<input name="name" autoComplete="name" placeholder="Sinu nimi" required maxLength={100}/></label><label>Ettevõte <span>(valikuline)</span><input name="company" autoComplete="organization" placeholder="Ettevõtte nimi" maxLength={150}/></label></div>
 <label>E-post<input name="email" type="email" autoComplete="email" placeholder="sina@ettevote.ee" required maxLength={200}/></label>
 ...
 <div className="form-bottom"><p>Andmeid kasutame päringule vastamiseks. <a href="/privaatsus">Privaatsustingimused</a></p><button className="button" disabled={pending} type="submit">{pending?'Saadan…':'Saada päring'} <span aria-hidden="true">↗</span></button></div><p role="status">{status}</p></form>;
}